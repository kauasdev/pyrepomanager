...
class GHConnect:
    token_file = "gh_user_token_file.txt"
    ...
    def __verify_or_create_token_file(self) -> str:
        if path.isfile(GHConnect.token_file):
            token = self.__read_token_file(GHConnect.token_file)
            if token != self.__gh_user_token:
                self.__write_token_file(GHConnect.token_file, self.__gh_user_token)
            else:
                pass
        else:
            self.__create_token_file(GHConnect.token_file, self.__gh_user_token)

        return GHConnect.token_file
